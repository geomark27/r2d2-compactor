...
**Distribución con FFmpeg incluido**: el asset principal es un **instalador NSIS** (`r2d2-compactor-setup.exe`, script en `installer/installer.nsi`, requiere `apt install nsis`) con `RequestExecutionLevel user` (sin admin). Por defecto instala en `C:\r2d2-compactor` (decisión del usuario del proyecto); la página de carpeta tiene una validación (`DirLeave`) que hace una prueba de escritura y rechaza con mensaje claro las rutas protegidas como Program Files — ahí, además de requerir admin, la auto-actualización (que reemplaza el `.exe` in situ) dejaría de funcionar. Crea acceso del menú Inicio, desinstalador y entrada en "Agregar o quitar programas" (HKCU). También se publica un **`.zip`** portable (`r2d2-compactor-windows-amd64.zip`) con la app + carpeta `ffmpeg/`. Los binarios de FFmpeg viven en `vendor/ffmpeg-win/` (gitignored, ~195 MB); se bajan una vez con `make vendor-ffmpeg`. `make dist-pkg` prepara `dist/pkg/` y de ahí salen `dist-windows` (zip) y `dist-installer` (setup).
